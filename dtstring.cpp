...
String& String::operator = (const char* s)
{
    if( m_str != s)
    {
        ...
        {
            THROW_EXCEPTION(NoEnoughMemoryException, "No memory to assign new String values...");
        }
    }
...
String String::sub(int i, int len) const
{
    ...
    if( (0 <= i) && (i < m_length) )
    {
        ...
        char* sub_str = reinterpret_cast<char*>(malloc(len + 1));

        strncpy(sub_str, m_str + i, len);
